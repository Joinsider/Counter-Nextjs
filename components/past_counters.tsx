...
import {fetchOldCounterValues} from '@/lib/store/slices/counterSlice';

interface CounterProps {
    typeId?: string;
}

export function PastCounters({typeId = '3bqw5z4ht16sz75'}: CounterProps) {
    const dispatch = useDispatch<AppDispatch>();
    const {history, isLoading, error} = useSelector((state: RootState) => state.counter);
    ...
    useEffect(() => {
        if (!isCollapsed) {
            // Only fetch data when the component is expanded
            dispatch(fetchOldCounterValues({
                typeID: typeId,
                month: currentMonth,
                year: currentYear
            }));
        }
    }, [typeId, currentMonth, currentYear, isCollapsed, dispatch]);
    ...
    const generateCalendarData = () => {
        // Create a map of dates with counter values from Redux history
        const counterMap = new Map();

        if (history && history.length > 0) {
            history.forEach(item => {
                counterMap.set(item.date, {value: item.value, date: item.date});
            });
        }
    ...
    return (
        <div className="mt-12">
            <h2 className="text-2xl font-semibold text-gray-700 dark:text-gray-300 mb-4 text-center cursor-default"
                    ...
                    <div className="space-y-4">
                        <div className="calendar-container p-4 bg-white dark:bg-gray-700 rounded-lg shadow">
                            ...
                            {isLoading ? (
                                ...
                                <div className="grid grid-cols-7 gap-1">
                                    ...
                                    {dayNames.map(day => (
                                        <div key={day}
                                             className="text-center font-medium text-gray-500 dark:text-gray-400 text-sm py-1">
                                            {day}
                                        </div>
                                    ))}
                                        ...
                                        >
                                            {dayData && (
                                                <>
                                                    ...
                                                    {/* Display counter value directly */}
                                                    {dayData.hasCounter && (
                                                        <span
                                                            className="absolute top-1 right-1 text-xs font-medium text-blue-500">
                                                                {dayData.counter.value}
                                                            </span>
                                                    )}
                                                    ...
                                                    {hoveredDate === dayData.date && dayData.hasCounter && (
                                                        <div
                                                            className="absolute z-10 bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-3 py-2 bg-gray-800 text-white text-sm rounded shadow-lg whitespace-nowrap">
                                                            {t('counter.pastCountersValue')}: {dayData.counter.value}
                                                            <div
                                                                className="absolute bottom-0 left-1/2 transform -translate-x-1/2 translate-y-1/2 rotate-45 w-2 h-2 bg-gray-800"></div>
                                                        </div>
                                                    )}
                                                    ...
                                                    {dayData.hasCounter && (
                                                        <div
                                                            className="absolute bottom-1 left-1/2 transform -translate-x-1/2 w-1.5 h-1.5 rounded-full bg-blue-500"></div>
                                                    )}
                                                </>
                                            )}
                                        </div>
                        ...
                        {error && (
                            <div className="text-center text-red-500">
                                Error loading counter history: {error}
                            </div>
                        )}
                    </div>
                )}
